...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "An AI scenic-route planner. Tell it your mood and your spare time; it plans a drive worth taking." },
    ],
  }),
  component: Landing,
});
...
function useIsAuthed() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data, error }) => {
      if (!mounted) return;
      if (error) {
        console.error("[Auth] home session check failed", error, error.stack ?? "Source line unavailable");
        setAuthed(false);
        return;
      }
      ...
      console.error("[Auth] home session check failed", exception.message, exception.stack ?? "Source line unavailable");
      setAuthed(false);
    });
    return () => {
      mounted = false;
    ...
    <div className="min-h-screen">
      ...
      <header className="mx-auto grid max-w-6xl grid-cols-[minmax(0,1fr)_auto] items-center gap-2 px-4 py-4 sm:flex sm:justify-between sm:px-6 sm:py-6">
        <Link to="/" className="flex min-w-0 items-center gap-2">
          ...
          <span className="truncate font-serif text-xl font-semibold tracking-tight text-ink">Scenik</span>
        </Link>
        <nav className="flex shrink-0 items-center gap-1 sm:gap-2">
          <Link to="/community">
            <Button variant="ghost" size="sm" className="px-2 font-medium sm:h-10 sm:px-4 sm:text-sm" aria-label="Community">
              <Users className="h-4 w-4 sm:mr-1.5" /><span className="hidden sm:inline">Community</span>
            </Button>
          </Link>
          {authed === false && (
            <Link to="/auth">
              <Button variant="ghost" size="sm" className="font-medium sm:h-10 sm:px-4 sm:text-sm">Sign in</Button>
            </Link>
          )}
          {authed === null ? (
      ...
      <section className="mx-auto max-w-3xl px-4 pt-10 pb-16 text-center sm:px-6 sm:pt-12 sm:pb-20">
        ...
        <p className="mx-auto mt-5 max-w-md text-base text-muted-foreground sm:mt-6 sm:text-lg">
          Tell Scenik your mood, your theme, and how many extra minutes
          you'll spare. We'll thread the prettiest drive between A and B —
          and score it out of 100.
        </p>
        <div className="mt-7 flex flex-col items-stretch justify-center gap-3 sm:mt-8 sm:flex-row sm:items-center">
          {authed === null ? (
            <Button size="lg" className="w-full shadow-stamp sm:w-auto" disabled>Checking account…</Button>
          ) : (
            <Link to="/plan" className="sm:w-auto">
              <Button size="lg" className="w-full shadow-stamp sm:w-auto">Plan my drive</Button>
            </Link>
          )}
          <Link to="/community" className="sm:w-auto">
            <Button size="lg" variant="outline" className="w-full sm:w-auto">
              <Users className="h-4 w-4 mr-2" />Browse community
            </Button>
          </Link>
        </div>
        ...
        <ul className="mx-auto mt-12 grid max-w-2xl gap-3 text-sm text-muted-foreground sm:grid-cols-3">
          {[
            ...
            <li key={f.t} className="rounded-2xl border border-border bg-card p-4 text-left shadow-paper">
      ...
      <section className="border-t border-border bg-parchment/60">
        <div className="mx-auto grid max-w-6xl gap-8 px-4 py-14 sm:gap-10 sm:px-6 sm:py-20 md:grid-cols-3">
          {[
            { n: "01", t: "Where & when", d: "Drop a start and end, and how many extra minutes you can spare." },
            { n: "02", t: "How it should feel", d: "Pick a mood and a theme — romantic coastal, adventurous historic, anything." },
            { n: "03", t: "Drive it", d: "We thread real waypoints between A and B with a scenic score and highlights." },
          ].map((s) => (
            <div key={s.n}>
              <div className="font-serif text-xs font-semibold uppercase tracking-[0.3em] text-primary">{s.n}</div>
      ...
      <footer className="border-t border-border">
        <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-6 text-xs text-muted-foreground sm:px-6 sm:py-8 sm:text-sm">
          <nav className="flex flex-wrap items-center gap-x-5 gap-y-2">
            ...
            <Link to="/refund" className="hover:text-ink">Refunds</Link>
