...
export const Route = createFileRoute("/community")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Discover scenic routes shared by other drivers — and share your own.",
      },
...
function CommunityPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      listFn({
        data: { sort, limit: 30, country: sort === "top" && country ? country : undefined },
      }),
  });

  const ids = (data ?? []).map((route) => route.id);
  ...
  const { data: myRatings } = useQuery({
    ...
    queryFn: () =>
      userId
        ? myRatingsFn({ data: { ids } })
        : Promise.resolve([] as Array<{ route_id: string; rating: number }>),
    enabled: !!userId && ids.length > 0,
  });
  const myRatingMap = new Map((myRatings ?? []).map((rating) => [rating.route_id, rating.rating]));
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        {!isLoading && (!data || data.length === 0) && (
          <Card className="mt-8 border-dashed border-border bg-card p-8 text-center shadow-paper sm:p-12">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Be the first — plan a route, save it, and toggle Share.
            </p>
            {userId ? (
              <Link to="/plan">
              ...
              <Link to="/auth">
                <Button className="mt-5 shadow-stamp">Sign in to share</Button>
              </Link>
            )}
          </Card>
        )}
        ...
        <div className="mt-6 grid gap-4 sm:mt-8 md:grid-cols-2">
          {data?.map((r) => {
            ...
            return (
              <Card
                key={r.id}
                className="border-border bg-card p-4 shadow-paper transition hover:shadow-stamp sm:p-5"
              >
                <Link to="/community/$id" params={{ id: r.id }} className="block">
                  ...
                  <p className="mt-3 line-clamp-2 font-serif text-sm italic text-ink/80">
                    "{r.narrative}"
                  </p>
                </Link>
