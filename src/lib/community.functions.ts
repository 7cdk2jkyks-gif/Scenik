...
const PUBLIC_ROUTE_COLS =
  "id, user_id, title, mood, theme, extra_minutes, start_address, end_address, start_lat, start_lng, end_lat, end_lng, waypoints, scenic_score, narrative, highlights, like_count, rating_avg, rating_count, created_at";
...
export const toggleRouteLike = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await context.supabase
      .from("route_likes")
      .insert({ user_id: context.userId, route_id: data.route_id });
    if (error) throw new Error(error.message);
    return { liked: true };
  });
...
export const myLikedRouteIds = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) =>
    z.object({ ids: z.array(z.string().uuid()).max(120) }).parse(input),
  )
  .handler(async ({ data, context }) => {
...
export const addRouteComment = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: unknown) =>
    z
      .object({ route_id: z.string().uuid(), body: z.string().trim().min(1).max(1000) })
      .parse(input),
  )
  .handler(async ({ data, context }) => {
    const { data: row, error } = await context.supabase
      .from("route_comments")
      .insert({ route_id: data.route_id, user_id: context.userId, body: data.body })
      .select()
      .single();
    if (error) throw new Error(error.message);
    return row;
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error: insErr } = await context.supabase
      ...
      .insert({
        ...
        mood: src.mood,
        theme: src.theme,
        extra_minutes: src.extra_minutes,
        start_address: src.start_address,
        end_address: src.end_address,
        start_lat: src.start_lat,
        start_lng: src.start_lng,
        end_lat: src.end_lat,
        end_lng: src.end_lng,
        waypoints: src.waypoints,
        scenic_score: src.scenic_score,
